...
/// `ClassDB.class_get_method_list(cls, true)` (own methods only — the `no_inheritance` flag set)
/// against the shipped stock dump for every class the dump carries, on the real Godot 4.6.3-stable
/// binary (the oracle technique in
...
/// from that same `class_get_method_list` entry's `flags` — NOT inferred from the `_`-prefix. Of the
/// 182 rows only 12 are `true` (the `Object`-core virtuals: `_get`/`_set`/`_init`/`_notification`/
...
///
/// To **regenerate** this table after the vendored stock dump is bumped to a new Godot version, run
/// `scripts/regen-dump-omitted-methods.sh <godot-binary>` against that version's binary and paste
/// its output between the `&[` / `];` markers below, then `cargo fmt --all`. The
/// `dump_omitted_*` tests in this crate and `dump_omitted_methods_*` in `gd_server` pin the table's
/// structural and against-the-dump invariants so a stale or corrupt edit fails CI (the regen itself
/// needs the binary, so it is a manual step — the tests are the binary-free drift tripwire). The
/// table is `pub` so the cross-dump invariant test in `gd_server` (where the real dump asset lives)
/// can read it; see [`crate::VARIANT_UTILITY_FUNCTIONS`] for the same pattern.
pub const DUMP_OMITTED_NATIVE_METHODS: &[(&str, &str, bool)] = &[
...
mod tests {
    ...
    // ── #172: binary-free drift/corruption tripwires for `DUMP_OMITTED_NATIVE_METHODS` ───────────
    // The table is mechanically regenerated by `scripts/regen-dump-omitted-methods.sh` (which needs
    // a Godot binary, so it cannot run in CI). These tests pin the table's *structural* invariants
    // so a stale hand-edit or a bad merge fails CI without the binary. The cross-dump invariants
    // (every class present in the stock dump; no row already in the dump's own-methods) live in
    // `gd_server`, where the real dump asset is reachable.

    /// The table is strictly sorted and deduplicated by `(class, method)` — the exact order the
    /// regen script emits. A reorder, duplicate, or out-of-order insertion (the classic bad
    /// merge / careless hand-edit) trips this.
    #[test]
    fn dump_omitted_table_is_strictly_sorted_and_deduped() {
        let keyed: Vec<(&str, &str)> = DUMP_OMITTED_NATIVE_METHODS
            .iter()
            .map(|&(c, m, _)| (c, m))
            .collect();
        for pair in keyed.windows(2) {
            assert!(
                pair[0] < pair[1],
                "DUMP_OMITTED_NATIVE_METHODS must be strictly sorted+deduped by (class, method); \
    ...
    /// No row carries an empty class or method name (a truncated paste / fat-finger).
    #[test]
    fn dump_omitted_rows_are_well_formed() {
        for &(class, method, _) in DUMP_OMITTED_NATIVE_METHODS {
            assert!(
                !class.is_empty() && !method.is_empty(),
                "empty class/method in DUMP_OMITTED_NATIVE_METHODS row: ({class:?}, {method:?})"
            );
        }
    }

    /// The omitted set is `free` (on `Object`) plus `_`-prefixed virtuals/internal methods — so the
    /// ONLY non-`_`-prefixed row is exactly `("Object", "free", false)`. A stray real method added
    /// by hand (the likeliest corruption — pasting an ordinary method into the table) trips this
    /// far more tightly than a generic prefix sweep would.
    #[test]
    fn dump_omitted_only_non_underscore_row_is_object_free() {
        let non_underscore: Vec<_> = DUMP_OMITTED_NATIVE_METHODS
            .iter()
            .filter(|&&(_, method, _)| !method.starts_with('_'))
            .collect();
        assert_eq!(
            non_underscore,
            vec![&("Object", "free", false)],
            "the only non-`_`-prefixed omitted method must be Object::free"
        );
    }

    /// `is_virtual == true` only for `_`-prefixed names — a one-directional sanity check (a real
    /// engine virtual is always `_`-prefixed; the converse does NOT hold — most `_`-prefixed rows
    /// are non-virtual internal `MethodBind`s, so this is NOT the derivation). Catches a row that
    /// flips a non-`_` method to virtual.
    #[test]
    fn dump_omitted_virtual_rows_are_underscore_prefixed() {
        for &(class, method, is_virtual) in DUMP_OMITTED_NATIVE_METHODS {
            if is_virtual {
                assert!(
                    method.starts_with('_'),
                    "is_virtual=true on a non-`_`-prefixed method ({class}::{method}) — \
                     a real engine virtual is always `_`-prefixed"
                );
            }
        }
    }
    ...
    fn dump_omitted_table_size_is_the_4_6_3_derivation() {
        assert_eq!(
            DUMP_OMITTED_NATIVE_METHODS.len(),
            182,
    ...
    /// The seed actually takes effect: a known omitted virtual the *raw* dump does NOT carry as a
    /// `_`-prefixed method resolves post-seed AND keeps its real `METHOD_FLAG_VIRTUAL` bit. Uses
    /// `_notification` against the trimmed fixture: that fixture lists `Object::notification`
    /// (no underscore) but NOT `Object::_notification`, so a resolving `_notification` proves the
    /// seed ran. (`free` is a FALSE probe here — the trimmed fixture already carries it, so the seed
    /// skips it; the full-dump `free` probe lives in the `gd_server` test.)
    #[test]
    fn dump_omitted_seed_takes_effect_on_trimmed_fixture() {
        let db = trimmed_db();
        let Some((_, NativeMember::Method(m))) = db.lookup_member("Object", "_notification") else {
            panic!("seeded `_notification` must resolve on Object post-seed");
        };
        assert!(
            m.is_virtual,
