...
//
// `tokens` must be the standalone `gd_syntax::tokenize` output for the same source the tree was
// parsed from (the classifier's precondition), so both run on `data`. We assert nothing about the
...
fuzz_target!(|data: &str| {
    ...
    let (tokens, _errs) = gd_syntax::tokenize(data);
