...
| Constant folding skips shared operands, so an invalid comparison between two *constant* shared operands takes the type path rather than the fold path | **A wording divergence, one row wide.** 4.6 folds the two operands and reports the constant template with the names `get_operation_type` coerced them to (`Invalid operands to operator <, Dictionary and Dictionary.` for `E < E` between two enum metas); 4.7 takes the type path and names the enum (`Invalid operands "f.gd.E" and "f.gd.E" for "<" operator.`). gdls materializes no shared value at either tag, so it emits the 4.7 wording at both. Detection parity holds; only the rendering of a diagnostic on an already-invalid line differs, and forcing the fold would mean inventing a `FoldedValue` for a Dictionary purely to render a worse message. Pinned by `comparison_operands.rs`. |
