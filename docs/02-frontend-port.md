...
A member's type crosses the seam as a `TypeExpr` — the declaration as written, not a resolved `DataType` — and `reducer::resolve_interface_type_expr` turns it into one against the reading file's own context. That function is the cross-file twin of `resolver::resolve_datatype` (analyzer.cpp:654-900) and answers the same heads in the same order: builtins, native classes, project `class_name`s, the declaring file's own enums and inner classes, `@GlobalScope` enums, and autoload singletons. It never diagnoses and never returns `Unresolved`; a head it cannot place degrades to `Variant`, because an interface gap must never read as a type error. What it cannot recover on its own is hardness, since a `TypeExpr` does not say whether it was written as an annotation, as `:=`, or as a plain `=`, so the interface carries that separately (`MemberFlags::ty_is_soft`, `01-architecture.md` §4) and the `var` arm applies it.
...
**The miss branch is one branch, and each arm proves its own claim.** Godot's `reduce_call` decides between `Function "%s()" not found in base %s.`, `Static function "%s()" not found in base "%s".`, and `UNSAFE_METHOD_ACCESS` inside a single `!found` branch (`gdscript_analyzer.cpp:3722-3774`), and its gates are kind-agnostic — any non-`self`, non-hard-`BUILTIN` base warns, and any non-super hard metatype gets the static error. gdls reaches the branch only where the miss is provable, which is a narrower gate per kind rather than a different shape: a `Class` or `Script` base must have had its ancestry walked end to end (`base_is_introspectable` — every in-file link resolved, every cross-file link's interface present and `parse_clean`, no cycle, a native root the DB knows), and the dump must be `Exact`, because the walk bottoms out in it. A `Native` metatype is left silent because Godot renders it as `GDScriptNativeClass`, a string gdls does not produce; a hard `Variant` base is left silent as a plain under-report.
