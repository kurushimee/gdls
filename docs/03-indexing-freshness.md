...
- **The `uid:// → res://` map**, scanned from the three places Godot writes a uid: the `*.uid` sidecar beside a script or a plain resource, the `[remap]` block of a `*.import` beside an imported asset, and the header line of a text `.tres`/`.tscn`, which declares its own. Godot dereferences a uid at the `FileAccess` layer, so every spelling that takes a `res://` path takes a `uid://` one too. The index holds the map and derefs inside `resolve_path`, ahead of the `res://` strip, which is what gives `preload("uid://…")`, a path-`extends`, the dependency edge, and autoload targets one answer from one place; the non-script `preload` typing path derefs through the same map before it reads an extension or an `.import`. Two inconsistencies are refused rather than guessed at: Godot's `uid://<invalid>` sentinel never enters the map, since letting it in would point every freshly created file at whichever one the walk reached last, and a uid two resources claim — what copying a file without re-importing leaves behind — resolves to neither. A uid nothing declares resolves to nothing and is never joined onto the root, so an unknown uid under-reports rather than inventing a target. One thing deliberately does *not* deref: `willRenameFiles` refuses a `uid://` literal, because a uid names the file by identity and a move leaves it correct — rewriting it to `res://` text would downgrade a stable reference, which the Godot editor never does.
...
**Open buffers are excluded at save time.** A file that is open in an editor buffer has no persisted stat entry, even when its disk bytes match the buffer: the buffer's interface may reflect unsaved text, so persisting the disk stat would let the next warm start serve the on-disk interface as if it were current ("never lie"). The exclusion is deliberate — save-time logs report it as `cache: excluding N open buffer(s) …` — and the consequence is that every file open at shutdown is re-parsed on the next start, which is the correct conservative behavior, not cache churn.

**Reading the warm-start counters.** The `warm_index: stat-diff complete:` line counts only `.gd`/`.tscn` files (`unchanged + reparsed + added + skipped` = walked scripts and scenes). Non-script assets are also stat-diffed — so their count appears in the `stat-diffing N cached files` line (the two agree when the on-disk asset set didn't change while gdls was off) — but they are path-indexed only, never parsed, and are reported separately as `A asset(s) stat-diffed` rather than folded into the script counters.
