...
**A member with no annotation is read off its initializer, and only the shapes that need no evaluation.** The shallow pass has no analyzer under it, so `var speed := compute_speed()` cannot be answered and is recorded as untyped. What it does decode is everything syntactically decided: literals, `Array`/`Dictionary` literals, a builtin constructor or constant, `A.new()` and `A.B.new()`, `x as T`, and `$Path`/`%Unique` (a bare `Node`, per `02-frontend-port.md` §11). This list is the floor for every cross-file read of that member — a shape missing from it reads as `Variant` from another file while Godot has a real type, and the access on it then goes unchecked, along with everything downstream. Two rules keep the floor honest. Under-reporting is the only safe direction, so a shape that could be decoded two ways is not decoded at all. And an inferred type is soft: Godot hands `var x = e` its `INFERRED` source and reserves the hard `ANNOTATED_INFERRED` for `:=` and for `const`, so the interface carries which one it was (`MemberFlags::ty_is_soft`) and the reader puts a soft type back down. Hardening an inferred member reports things Godot does not.
