...
**A member with no annotation is read off its initializer, in two steps.** The shallow pass decodes everything syntactically decided into a type on the spot: literals, `Array`/`Dictionary` literals, a builtin constructor or constant, `A.new()` and `A.B.new()`, `x as T`, and `$Path`/`%Unique` (a bare `Node`, per `02-frontend-port.md` §11). What it cannot decode it *records*, as `InitShape` — a dotted chain read as a value (`SOME`, `E.A`, `Other.KONST`, `SomeAutoload.level`), a call whose callee is such a chain (`make()`, `Other.make()`), or a `preload` of a string literal — `res://`, `uid://`, or written relative to the reading file (the index joins the relative form against that file's own directory when it walks the dependency edges). A shape nests: either of the first two may hang off the result of another one, so `OS.get_temp_dir().path_join("x")` and `preload("res://a.gd").new().tag` are recorded whole, up to `INIT_SHAPE_MAX_DEPTH` links. What still has more than one reading is refused whole rather than truncated — an index, a call through a value, a call whose result depends on its arguments — because a wrong cross-file type is worse than none. The reading file's analyzer resolves that shape lazily against the declaring class, through interfaces only, so nothing has to run the other file's analysis and nothing crosses that a later edit could make stale without invalidating. A defaulted `func` parameter takes the same two steps, in `MemberDecl::param_inits` (`02-frontend-port.md` §12).

A `preload` that names something other than a project script is typed from the resource itself, in the order Godot's own `type_from_variant` would reach: a `uid://` argument is dereferenced through the project's uid map first, then a text `.tres`/`.tscn` answers with the class its header line names (`script_class` before `type`, since a resource carrying a script is an instance of that script), then an imported asset answers with the `type=` of its `.import`, and the extension map's `PackedScene`/`Shader`/`Resource` is the floor beneath all of it. Every answer is gated on the class being one the loaded dump carries, so an unrecognized name degrades to `Variant` rather than becoming something nothing can resolve. Hover reads the same resolved type off the reading file's own type table, which is what keeps a member card and the diagnostics on that member from disagreeing.

A `func` parameter with no annotation is read the same way, since Godot resolves one through the very same `resolve_assignable`. The shallow pass decodes `f(a := "")` to `String` and records, per parameter, how it got there — annotated, inferred by `:=` (hard), inferred by `=` (soft), genuinely untyped, or a default it could not read (`ParamTyping`). The last two are different answers and must not be confused: an untyped parameter really is a soft `Variant` and a caller's argument check names it that way, while an undecodable default means the declaring file has a type gdls has not read, so the reader carries the no-type dummy and the argument checks stay off it.
