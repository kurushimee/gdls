...
| **M7** | Phase 2 — protocol foundations | Concurrent dispatch + `$/cancelRequest` *preemption* (`RequestCancelled`/`ContentModified`); `workDoneProgress`; `workspace/didChangeConfiguration` + `workspace/configuration`; dynamic `didChangeWatchedFiles` registration; pull diagnostics (`textDocument/diagnostic`); `##` doc-comment + BBCode→Markdown pipeline; diagnostic `codeDescription` | Full spec: [`09-phase-2.md`](09-phase-2.md) §6. |
| **M8** | Phase 2 — editing core | `textDocument/completion` + `completionItem/resolve`; `textDocument/signatureHelp` — full convention set (CompletionList, textEdit, sortText, snippet/commit gating) | [`09-phase-2.md`](09-phase-2.md) §6-M8; the Phase 2 long pole. |
| **M9** | Phase 2 — navigation & refactoring | `rename`/`prepareRename`; `documentHighlight`; `declaration`/`typeDefinition`; `typeHierarchy`; `foldingRange`; `selectionRange`; `workspaceSymbol/resolve` | [`09-phase-2.md`](09-phase-2.md) §6-M9. |
| **M10** | Phase 2 — presentation & actions | `semanticTokens` full/delta/range (standard legend only); `inlayHint`; `documentColor`; `codeAction` quickfixes + `source.fixAll` | [`09-phase-2.md`](09-phase-2.md) §6-M10. |
| **M11** | Phase 2 — scenes & file operations | `.tscn` scene index → precise `$`/`%` typing (retires the `docs/02 §11` deviation); scene-aware node-path completion; autoload scene typing; `willRenameFiles` path edits; external-formatter bridge | `$`/`%` diagnostics converge with Godot on both acceptance projects. [`09-phase-2.md`](09-phase-2.md) §6-M11. (The persistent warm-start index cache moved **into M6** — it gated v1; see the M6 row.) |
...
code-actions/semantic-tokens **in v1** (all are Phase 2 scope — [`09-phase-2.md`](09-phase-2.md); formatting
only as an external-command bridge), any GUI/debugger, and any dependency on a running Godot process.
Permanently out of scope across all phases: Godot's custom LSP protocol extensions (`gdscript/*`,
`gdscript_client/*`) — see the anti-catalog in [`09-phase-2.md`](09-phase-2.md) §3.
