...
in CI; verification is the Phase H manual walk, not a synthetic soak). The v1 ship-gate was the
...
bar. The walk caught one v1 blocker the trimmed conformance fixture couldn't reach —
...
WP-D1 differential-harness construction bugs) plus a follow-up backlog are tracked in
`docs/08-phase1-remaining.md`; the optional WP-D1 local differential passes at mean Jaccard 0.8667 ≥ 0.85.
...
- **Calibration from M3 (closed at 1.0):** the analyzer port shipped in ~70 phased
  ...
  WP. Budget M4/M5/Phase 2 with the same fidelity-bench-driven follow-on assumption rather than the
